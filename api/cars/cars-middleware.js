...
exports.checkCarId = (req, res, next) => {
  db.getById(req.params.id)
    .then(car => {
      if(!car) return next({status: 404, message: `car with id ${req.params.id} is not found`})
        req.car = car
        next()
      })
...
exports.checkCarPayload = (req, res, next) => {
  ...
  if(!transmission) return next({ status: 400, message: `transmission is missing` })
  next();
}
...
exports.checkVinNumberValid = (req, res, next) => {
  ...
  const { vin } = req.body
  if(!vinValidator.validate(vin)) return next({status: 400, message: `vin ${req.body.vin} is invalid`})
  // req.car = car;
  next()
}
...
exports.checkVinNumberUnique = (req, res, next) => {
  ...
  db.get({vin: req.body.vin})
    .then(car => {
      if(car.length > 0) return next({status: 400, message: `vin ${req.body.vin} already exists`})
      next()
    })
}
